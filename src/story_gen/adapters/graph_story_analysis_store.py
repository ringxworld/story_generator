"""Prototype graph-style analysis store adapter with traversal index sidecar."""
...
def _build_adjacency_index(dashboard: dict[str, object]) -> dict[str, list[dict[str, str]]]:
    ...
    for item in raw_edges:
        ...
        if not source or not target:
            continue
        adjacency.setdefault(source, []).append({"node": target, "relation": relation})
        adjacency.setdefault(target, []).append({"node": source, "relation": relation})
    return adjacency


class GraphStoryAnalysisStore:
    """Persist analysis payloads with a graph adjacency sidecar for traversal work."""

    def __init__(self, db_path: Path) -> None:
        self._runs_path = db_path.with_name(f"{db_path.stem}.graph_analysis_runs.jsonl")
        self._meta_path = db_path.with_name(f"{db_path.stem}.graph_analysis_meta.json")
    ...
    def write_analysis_result(
        ...
        *,
        ...
        """Persist one run and store an adjacency index for future graph traversals."""
        ...
        record = {
            ...
            "adjacency_index": _build_adjacency_index(dashboard),
        }
    ...
    def get_latest_analysis(
        ...
        *,
        ...
        """Load latest analysis for one owner/story from append-only records."""
        ...
        for raw in reversed(lines):
            ...
            if not isinstance(dashboard_payload, dict):
                raise RuntimeError(
                    "Invalid dashboard payload in graph prototype store: expected object."
                )
